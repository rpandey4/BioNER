...
def get_negative_combination_sequences(original_replaced_sent, tmp_combinations_dict, SWAP_DICT, kkk=5, max_tokens=24):
    doc = nlp(original_replaced_sent)
    token_to_change = []
    concept_type = SWAP_DICT[0]
    ...
    for cmb_list in combinations:
        ...
        for j in range(len(token_to_change)):
            try:
                if cmb_list[j] == 1:
                    tmp_sent[token_to_change[j]] = concept_type
...
def get_cdr_masked_lm_with_other(file_path, output_folder, tmp_combinations_dict, data_type, concept_type, MAX_TOKENS, replace_type="all"):
    sentences = []
    sentences_replaced = []
    sentences_negative = []
    cur_sent = []
    ...
    for lines in open(file_path):
        ...
        if lines == "":
            ...
            # print(cur_sent)
            # print(cur_sent_replaced)
            # print(cur_sent_negative)
            # print("masked dict", masked_dict)
            # print("\n%s\n" % ("*"*100))
            tot_sent += 1
            # if tot_sent == 5:
            #     sys.exit()
            sentences_replaced.append(" ".join(cur_sent_replaced))
            sentences_negative.extend(get_negative_combination_sequences(" ".join(cur_sent_replaced), tmp_combinations_dict, SWAP_DICT, kkk=10, max_tokens=MAX_TOKENS))
        ...
        if lines[1] == "O":
            ...
            masked_dict[masked_ind].append(lines[0])
    # for i in range(len(sentences)):
    sentences_negative.extend(sentences)
...
if __name__ == "__main__":
    ...
    OUT_PATH = "/Users/rahulpandey/OneDrive - Philips/philips/rahul-ai-work/clinical_trial_disambiguation/data/bert-ready/MTL-Bioinformatics/Appropriate/NO_OTHERS/ALL_NEGATIVES/BC5CDR"
    ...
    get_cdr_masked_lm_with_other(os.path.join(INP_PATH, "BC5CDR-disease-IOB", "train.tsv"), OUT_PATH, tmp_combinations_dict, "train", "disease", MAX_TOKENS, "disease")
    get_cdr_masked_lm_with_other(os.path.join(INP_PATH, "BC5CDR-chem-IOB", "train.tsv"), OUT_PATH, tmp_combinations_dict, "train", "chemical", MAX_TOKENS, "chem")
    # get_cdr_masked_lm_with_other(os.path.join(INP_PATH, "BC5CDR-IOB", "devel.tsv"), OUT_PATH, tmp_combinations_dict, "dev", "all")
    get_cdr_masked_lm_with_other(os.path.join(INP_PATH, "BC5CDR-disease-IOB", "devel.tsv"), OUT_PATH, tmp_combinations_dict, "dev", "disease", MAX_TOKENS, "disease")
    get_cdr_masked_lm_with_other(os.path.join(INP_PATH, "BC5CDR-chem-IOB", "devel.tsv"), OUT_PATH, tmp_combinations_dict, "dev", "chemical", MAX_TOKENS, "chem")
